...
// Потолок длины исходника страницы. Живёт здесь, а не в server.js: тот же
// предел действует и в API записи, а правило должно быть одно на всех.
export const MAX_CONTENT = 200000;
...
// закрывают куски слов и числа, [ДАННЫЕ УДАЛЕНЫ] — всё остальное: слово,
// фразу, абзац, закрытый блоком раздел.
// ---------------------------------------------------------------------------
const BLOCK = '█';
// Плашка у скрытого текста одна на все случаи: разнобой из «[УДАЛЕНО]»,
// «[ЗАСЕКРЕЧЕНО]» и «[ДОСТУП К РАЗДЕЛУ ОГРАНИЧЕН]» рябил в глазах на страницах,
// где скрыто почти всё. Закраска ████ осталась там, где фраза не к месту:
// на куске слова и на числах.
const REDACTED = '[ДАННЫЕ УДАЛЕНЫ]';
const NOTICE = REDACTED;
const SECTION_NOTICE = REDACTED;
...
// Эти поля в карточке не печатаются вовсе — ни вошедшему, ни постороннему.
// Числовой телеграм-айди нужен только боту через API, человеку на странице он
// не нужен, а «спрятать в карточке» мало: строка выдала бы сам факт и место.
const HIDDEN_KEYS = ['телеграм_id', 'telegram_id'];
...
export function renderInfobox(meta, title, env) {
  ...
  for (const [key, value] of entries) {
    ...
    // Скрытое поле не даёт строки карточки никому: значение живёт в источнике
    // (обёрнутым в {{…}}, чтобы не утечь в поиск и диффы) и читается через API.
